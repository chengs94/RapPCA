sim_y = function(dim_x, n_pc, n_noise, dim_y, 
  ...
  if (krige){
    ...
    # pcs = cbind(f_xb, matrix(0, nrow = n, ncol = n_noise)) +
    #   t(mvtnorm::rmvnorm(n_pc+n_noise, sigma = pc_cov))
    bb = NULL
  } else {
    ...
    # bb = matrix(runif(4*(n_pc+n_noise), 0, 1), nrow = 4)
    # pcs = cbind(f_xb, matrix(0, nrow = n, ncol = n_noise)) + cov_sill* Btrn$X %*% bb +
    #   matrix(rnorm(n*(n_pc+n_noise), 0, cov_nugget), nrow = n)
    pc_cov = NULL
  }
  ...
  # if (decay_v) v_raw = v_raw * seq(1,(n_pc+n_noise),1)^2
...
prpca_one = function(x, y, coords, r, lambdas, lambda.ratios, gms, 
  ...
  while (r > 0){
    ...
    # .rslt = rep_predPCA(xtrn = x.scaled[-tst.ind,], xtst = x.scaled[tst.ind,], 
    #                     ytrn = y.scaled[-tst.ind,], ytst = y.scaled[tst.ind,], 
    #                     coords.trn = coords[-tst.ind,], coords.tst = coords[tst.ind,],
    #                     lambda = lambda, lambda.ratio = lambda.ratio, 
    #                     gm = gm, d = d, m = m)
...
predpca_one = function(x, y, coords, r, #lambda2s, nvars,
  ...
  # pars2 = expand.grid(lambda2 = lambda2s, nvar = nvars)
  # pars2$tmse = pars2$pc.mse = NA
  cov.given = !is.null(x)
